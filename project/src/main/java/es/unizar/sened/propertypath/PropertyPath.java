...
// TODO revisar descripción (descripciones en inglés?)

/**
 * Camino de propiedades <br>
 * Destinado a crear consultas específicas para búsquedas de refinado (léase
 * {@link JQueryFactory#getSpecificQuery(String, PropertyPath)}) <br>
 * Su funcionamiento es el siguiente: <br>
 * El {@link OntologyManager} contiene dos (en términos generales) ontologías, una interna (con el namespace igual a la
 * ruta del fichero) y una externa, que es un subconjunto de las ontologías usadas en la DBPedia. <br>
 * Cuando se llama al constructor de PropertyPath hay que darle 2 {@link OWLClass} pertenecientes a la ontología
 * "interna", entonces se nos creará un camino de propiedades "externas" que enlace las dos clases elegidas. <br>
 * Se podría decir que éste objeto es el paso intermedio entre las ontologías "interna" y "externa" y el que se ocupa de
 * ocultar la ontología "interna" para no tener que preocuparnos por ella. <br>
 * Nota: "interna" y "externa" van entre comillas ya que realmente ambas están presentes en el fichero local a cargar,
 * sin embargo la "externa" es la que contiene las mismas clases y propiedades que la DBPedia.
 * 
 * @author gesteban@unizar.es
 */
// TODO remove this class and make it easier/simple.
public class PropertyPath extends ArrayList<DirectionalProperty> {

	private static final long serialVersionUID = -6419648876757159287L;

	public static final String TAG = PropertyPath.class.getSimpleName();
	...
	static public Set<PropertyPath> createSimplePaths(OWLClass source, OWLClass target) {
		...
		Log.d(TAG, "<createSimplePaths> Set<PropertyPath> de " + source.getIRI().getFragment() + " a "
				+ target.getIRI().getFragment() + " creado con " + pathSet.size() + " PropertyPath/s");
	...
	static public Set<PropertyPath> createComplexPaths(OWLClass source, OWLClass target, int deep) {
		...
		for (OWLObjectProperty prop : om.getSubsetExternal(OWLObjectProperty.class,
				om.getObjectPropertiesByDomain(source))) {
			if (!om.isDataRetrievable(prop))
				set1.add(new PropertyPath(source, target, new DirectionalProperty(prop, true)));
		}
		for (OWLObjectProperty prop : om.getSubsetExternal(OWLObjectProperty.class,
				om.getObjectPropertiesByRange(source))) {
			if (!om.isDataRetrievable(prop))
	...
	private Set<PropertyPath> expandPath() {
		...
		for (OWLClass clase : classSet) {
			for (OWLObjectProperty prop2 : om.getSubsetExternal(OWLObjectProperty.class,
					om.getObjectPropertiesByDomain(clase))) {
				if (!om.isDataRetrievable(prop2)) {
			...
			for (OWLObjectProperty prop2 : om.getSubsetExternal(OWLObjectProperty.class,
					om.getObjectPropertiesByRange(clase))) {
				if (!om.isDataRetrievable(prop2)) {
					PropertyPath newPP = (PropertyPath) this.clone();
	...
	public static void main(String[] args) {
		...
		Set<PropertyPath> pps = PropertyPath.createSimplePaths(
				fac.getOWLClass(IRI.create("file:/c:/users/peonza/documents/proyecto/subsets_merged.owl#Articulo")),
				fac.getOWLClass(IRI.create("file:/c:/users/peonza/documents/proyecto/subsets_merged.owl#Categoria")));
