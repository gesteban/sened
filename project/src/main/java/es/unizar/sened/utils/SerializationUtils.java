...
public class SerializationUtils {
				...
				for (int i = 0; i < rs.getResultSize(); i++) {
					...
					if (n == null) {
						...
						for (int j = 0; j < rs.getColumnCount(); j++) {
							Element property = doc.createElement(rs
									.getColumnName(j));
						...
						for (int j = 0; j < rs.getColumnCount(); j++) {
							if (!existNode(parent, rs.getColumnName(j),
									rs.getElement(i, j))) {
								Element property = doc.createElement(rs
										.getColumnName(j));
								parent.appendChild(property);
								Text text = doc.createTextNode(rs.getRow(i)
										.get(j));
								property.appendChild(text);
							}
						}
	...
	private static boolean existNode(Node node, String nodeName,
			String textValue) {
